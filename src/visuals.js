...
export function playerCard({ uid, player: p, index, self, selected, chosen, connected = true, winner = false, targetable = false, rules, effects = {} }) {
  ...
  return `<button class="player ${self ? 'self' : ''} ${winner ? 'winner' : ''} ${targetable ? 'targetable' : ''} ${chosen ? 'has-chosen' : ''} ${p.hair === 0 ? 'eliminated' : ''} ${p.hair === 1 ? 'critical' : ''} ${!connected ? 'offline-player' : ''} ${selected ? 'selected-target' : ''} ${effectClass}" data-player="${esc(uid)}" aria-pressed="${selected ? 'true' : 'false'}" aria-label="${esc(p.name)}${self ? ', vos' : ''}. ${p.hair} Pelo, ${p.breath} Soplos${targetable ? '. Elegir como objetivo' : ''}" style="--seat-color:${colors[seat % colors.length]};--hair-fill:${hairFill};--breath-fill:${breathFill}" ${p.hair <= 0 || self || !targetable ? 'disabled' : ''}>
...
export function actionControls(enabled, breath, aiming, selectedAction = null, hideLocked = false, saving = false) {
  ...
  return `<div class="controls" aria-label="Elegí tu acción">
    <button id="blow" aria-label="${breath < 1 ? 'Soplar: necesitás 1 Soplo' : aiming ? 'Cancelar selección de objetivo' : 'Soplar'}" class="${aiming ? 'aiming ' : ''}${selectedClass('blow')}" aria-pressed="${pressed('blow')}" ${!enabled || breath < 1 ? 'disabled' : ''}>${icons.blow}<span>Soplar</span>${state('blow')}</button>
