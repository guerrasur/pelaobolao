...
function render() {
  ...
  if (s.updateRequired) {
    ...
    html = `<section class="lobby-screen"><div class="lobby-heading"><div><h1><span class="code">${esc(s.room.code)}</span></h1></div><button id="share-room" class="secondary compact-button">Compartir</button></div><div class="ready-progress" style="--ready:${readyRatio}"><div aria-hidden="true"><i></i></div><span>${readyCount}/${members.length} listos · ${onlineCount} conectados</span></div><ul class="lobby-list">${members.map(([uid, m], index) => `<li class="${[m.ready ? 'is-ready' : '', memberOnline(uid) ? '' : 'is-offline'].filter(Boolean).join(' ')}" data-member="${esc(uid)}" data-ready="${m.ready ? 'true' : 'false'}"><div class="lobby-player"><strong><span class="lobby-slot" aria-hidden="true">${index + 1}</span><span class="lobby-player-name">${esc(m.name)}</span>${uid === api.uid ? '<span class="lobby-self-tag">VOS</span>' : ''}</strong><span class="presence-text ${memberOnline(uid) ? 'is-online' : ''}" data-presence="${esc(uid)}">${memberOnline(uid) ? 'Conectado' : 'Reconectando…'}</span></div><span class="lobby-state">${uid === s.room.hostId ? '<em>HOST</em>' : ''}<b>${m.ready ? 'LISTO' : 'NO LISTO'}</b></span></li>`).join('')}</ul><button id="ready-toggle" class="${s.room.members[api.uid].ready ? 'secondary ready-toggle-on' : ''}" ${disabled}>${s.room.members[api.uid].ready ? '✓ Estoy listo' : 'Estoy listo'}</button>${host ? `<button id="start-game" ${disabled || (members.length < 2) || missingReady > 0 || offlineCount > 0 ? 'disabled' : ''}>Iniciar partida</button>` : ''}<button id="leave-room" class="quiet">Salir</button></section>`;
  } else if (!s.game) {
