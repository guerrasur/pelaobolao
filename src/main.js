...
function render() {
  ...
  if (s.updateRequired) {
    ...
    html = '<section class="state entry-screen entry-loading"><div class="entry-loader" aria-hidden="true"><i></i><i></i><i></i><b></b></div><p class="entry-step">ABRIENDO EL AULA</p><h1>Preparando tu jugador…</h1><p class="muted">Acomodando bancos, soplos y tu lugar en la ronda.</p><button id="reload-app" class="quiet">Reintentar</button></section>';
  } else if (!s.nameConfirmed) {
    html = `<section class="entry-screen profile-screen"><div class="entry-note-pin" aria-hidden="true"></div><p class="entry-step">TU FICHA · 01</p><div class="profile-heading"><div class="profile-doodle" aria-hidden="true"><i></i><b></b><b></b><b></b></div><div><h1>¿Cómo te llamás?</h1><p>Ese nombre va a aparecer en tu banco durante la partida.</p></div></div><form id="profile-form" class="entry-form"><label for="player-name">Nombre del jugador</label><input id="player-name" name="name" maxlength="24" required autocomplete="nickname" placeholder="Tu nombre" value="${esc(s.profile?.name ?? '')}"><button ${disabled}><span>Entrar al aula</span><small>Podés cambiarlo la próxima vez</small></button></form><p class="entry-footnote">Dejamos precargado el último nombre usado en este navegador.</p></section>`;
  } else if (!s.roomId) {
    const joinCode = new URLSearchParams(location.search).get('s') ?? '';
    html = `<section class="entry-screen home-screen"><div class="home-hero"><p class="entry-step">AULA ABIERTA · 2–6 JUGADORES</p><div class="home-copy"><p class="eyebrow">Hola, ${esc(s.profile.name)}</p><h1>Que no te vuelen <em>el pelo.</em></h1><p class="home-subtitle">Elegí en secreto. Soplá, escondete o tomá aire. Cada uno juega desde su celular.</p></div><div class="home-mascot" aria-hidden="true"><span class="mascot-hair"><i></i><i></i><i></i></span><span class="mascot-head"><b></b><b></b></span><span class="mascot-gust"><i></i><i></i><i></i></span></div></div><div class="home-actions"><button id="create-room" class="home-create" ${disabled}><span>CREAR SALA</span><small>Armá una partida nueva</small></button><form id="join-form" class="join-card"><div class="join-heading"><label for="room-code">¿Ya tienen sala?</label><span>Meté el código de 4 caracteres</span></div><div class="join-row"><input id="room-code" name="code" maxlength="4" minlength="4" pattern="[A-Za-z2-9]{4}" value="${esc(joinCode)}" placeholder="AB7K" autocapitalize="characters" autocomplete="off" spellcheck="false" required aria-label="Código de sala"><button class="secondary" ${disabled}><span>ENTRAR</span></button></div></form></div><p class="entry-footnote home-footnote"><span aria-hidden="true">●</span> Partidas rápidas · sin cuenta · mismo aula, distintos celulares</p></section>`;
  } else if (!s.room) {
    html = '<section class="state entry-screen room-loading"><div class="entry-loader door-loader" aria-hidden="true"><i></i><i></i><i></i><b></b></div><p class="entry-step">ABRIENDO LA PUERTA</p><h1>Entrando a la sala…</h1><p class="muted">Buscando tu banco y sincronizando el aula.</p><button id="reset-session" class="quiet">Volver al inicio</button></section>';
