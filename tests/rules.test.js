...
test('intención de agarrar acepta el mechón sin Soplos y Soplar ya no puede apuntarle',async()=>{
  await env.withSecurityRulesDisabled(async ctx=>{
  ...
  await env.withSecurityRulesDisabled(ctx=>updateDoc(doc(ctx.firestore(),'games',gameId),{'players.bob.breath':1}));
  ...
  await assertFails(setDoc(doc(b,'games',gameId,'intents','bob'),{
    turn:1,action:'grab',target:'__center_item__',requestId:'item-missing',revision:5,submittedAt:serverTimestamp(),
  }));
});
