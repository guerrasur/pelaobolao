...
test('+1 Pelo aparece como mechón flotante y se agarra gratis sin mover tarjetas', async () => {
  ...
  const itemMarkup = html.match(/<button[^>]*class="center-item hair-item[\\s\\S]*?<\\/button>/)?.[0] ?? '';
  assert.ok(itemMarkup);
  assert.doesNotMatch(itemMarkup,/<small|<strong|<span|item-new-badge/);
  assert.match(itemMarkup,/aria-label="\+1 Pelo\. Agarrar no cuesta Soplos, pero te deja expuesto\./);
...
test('Plan Condor 0.18: elegir Agarrar usa estado verde y al apuntar Soplar deshabilita el mechón', async () => {
  ...
  assert.match(html,/class="hair-tuft"/);
  assert.doesNotMatch(html,/>YENDO…<\/span>/);
...
test('Plan Aguila 0.17: el item explica riesgo gratuito y no se renderiza encima del final de partida', async () => {
  ...
  assert.doesNotMatch(html,/item-new-badge/);
  assert.doesNotMatch(html,/class="item-notice/);
