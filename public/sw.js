const CACHE = 'pelaobolao-shell-0.38.0';
const CORE = ['/', '/manifest.webmanifest', '/icon-192.png', '/icon.svg', '/assets/menu-pelao-bolao.webp'];
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(key => key.startsWith('pelaobolao-shell-') && key !== CACHE).map(key => caches.delete(key))))
