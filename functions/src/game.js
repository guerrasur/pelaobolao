// Server-only rules. A match snapshots these rules, so later edits affect new games only.
...
export function newGame(roomId, members, now, rules = RULES) {
  ...
  return {
    schemaVersion: 1, roomId, memberIds: ids, rules: { ...rules },
...
// Pure function called ONLY by the server. All actions use the pre-turn snapshot.
...
export function pruneLobby(room, now) {
  const members = Object.fromEntries(Object.entries(room.members)
    .filter(([, member]) => !member.left && now - member.lastSeenAt < LOBBY_LEASE_MS));
